...
const securityMiddleware = async ( req: Request, res: Response, next: NextFunction) => {
    ...
    if (process.env.NODE_ENV === "test" || process.env.ARCJET_ENV === "development") return next();
    try {
        ...
        switch (role) {
            case "admin":
                limit=20;
                message = "Admin request limit exceeded (20 per minute). Slow down Sir!";
                break;
            case "teacher":
            case "student":
                limit=10;
                message = "User request limit exceeded (10 per minute). Please wait my dear!";
                break;
            default:
                limit=5;
